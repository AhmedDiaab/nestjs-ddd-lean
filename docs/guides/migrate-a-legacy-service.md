...
- Request and response bodies stream through unchanged (`node:http`/`node:https`, never buffered); nothing is parsed or re-wrapped in the envelope, and no body, header or query string is ever logged.
...
- The forwarder has its own timeout, `LEGACY_TIMEOUT_MS`, kept below the VIP's; a failure of this hop logs `legacy.forward.failed` with the method, path and status, never the body or headers.
- Forwarded traffic gets its own log file (`LOGGING_DIR/LEGACY_LOG_FILE_NAME`, default `logs/legacy-forward.log`), because answering in front of Nest's router keeps these requests out of the normal access log entirely. Exactly one line per request — `legacy.forward.completed`, `legacy.forward.aborted` or `legacy.forward.failed`, with the method, path, status, request id and latency — is the ledger of what is still not migrated, and it shrinks to nothing as the prefixes do ([Logging → The legacy forwarding log](../architecture/logging.md#the-legacy-forwarding-log)).

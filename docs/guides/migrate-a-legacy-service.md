...
- Forwarded traffic gets its own log file (`LOGGING_DIR/LEGACY_LOG_FILE_NAME`, default `logs/legacy-forward.log`), because answering in front of Nest's router keeps these requests out of the normal access log entirely. One `legacy.forward.completed` line per request — method, path, status, request id, latency — is the ledger of what is still not migrated, and it shrinks to nothing as the prefixes do ([Logging → The legacy forwarding log](../architecture/logging.md#the-legacy-forwarding-log)).

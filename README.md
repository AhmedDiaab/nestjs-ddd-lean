...
| **Cluster mode**           | optional multi-core scaling on one box via Node's built-in `cluster` module (`CLUSTER_ENABLED`, off by default); the primary forks/respawns workers, elects a scheduler leader, forwards shutdown signals explicitly, and a boot-time rail logs pool capacity before workers start |
